...
var coll_frontWheel : WheelCollider;		//碰撞器
var coll_rearWheel : WheelCollider;
// visual for wheels
var meshFrontWheel : GameObject;			//模型
...
var crashAngle01: float;//crashed status is on if bike have more Z(side fall) angle than this				纵向crash角度范围								// 70 sport, 55 chopper
var crashAngle02: float;//crashed status is on if bike have less Z(side fall) angle than this 												// 290 sport, 305 chopper
var crashAngle03: float;//crashed status is on if bike have more X(front fall) angle than this 				横向crash角度范围								// 70 sport, 70 chopper
var crashAngle04: float;//crashed status is on if bike have more X(back fall) angle than this												// 280 sport, 70 chopper
											
// define CoM of bike
var CoM : Transform; //CoM object		center of mass重心
var normalCoM : float; //normalCoM is for situation when script need to return CoM in starting position			初始的com							// -0.77 sport, -0.38 chopper
...
var rearPendulumn : Transform; //rear pendulumn 模型给的是固定后轮的架子，后方避震？
var steeringWheel : Transform; //wheel bar 车把
var suspensionFront_down : Transform; //lower part of front forge 避震器
private var normalFrontSuspSpring : int; // we need to declare it to know what is normal front spring state is 前端正常悬浮高度
private var normalRearSuspSpring : int; // we need to declare it to know what is normal rear spring state is 后轮正常悬浮高度
...
private var baseDistance : float; // need to know distance between wheels - base. It's for wheelie compensate(dont want wheelie for long bikes)前后轮距离，前轮离地特技的补偿
...
var wheelbarRestrictCurve : AnimationCurve = new AnimationCurve(new Keyframe(0f, 20f), new Keyframe(100f, 1f));//first number in Keyframe is speed, second is max wheelbar degree	控制车把转动幅度的曲线，速度越大转动角度越小

// temporary variable to restrict wheel angle according speed		临时变量，根据速度约束轮子的角度
private var tempMaxWheelAngle : float;

//variable for cut off wheel bar rotation angle at high speed		高速情况下车把的可能角度
private var wheelPossibleAngle : float = 0.0;

//for wheels vusials match up the wheelColliders		轮子的中心，为了向子物体提供match UP的标准？
private var wheelCCenter : Vector3;
private var hit : RaycastHit;

/////////////////////////////////////////// technical variables ///////////////////////////////////////////////////////
static var bikeSpeed : float; //to know bike speed km/h
static var isReverseOn : boolean = false; //to turn On and Off reverse speed		是否反转速度
// Engine
var frontBrakePower : float; //brake power absract - 100 is good brakes				制动力														// 100 sport, 70 chopper
var EngineTorque : float; //engine power(abstract - not in HP or so)				引擎扭矩														// 85 sport, 100 chopper
// airRes is for wind resistance to large bikes more than small ones				空气阻力
var airRes : float; //Air resistant 																										// 1 is neutral
/// GearBox
var MaxEngineRPM : float; //engine maximum rotation per minute(RPM) when gearbox should switch to higher gear 	引擎在该档下，最大的转速							// 12000 sport, 7000 chopper
var EngineRedline : float; 																													// 12200 sport, 7200 chopper
var MinEngineRPM : float; //lowest RPM when gear need to be switched down		引擎换低档时的最小转速															// 6000 sport, 2500 chopper
static var EngineRPM : float; // engine current rotation per minute(RPM)		当前引擎转速
// gear ratios(abstract)
var GearRatio: float[];//array of gears          各个挡位                                                                                       	// 6 sport, 5 chopper

var CurrentGear : int = 0; // current gear		当前挡位

//private var ctrlHub : GameObject;// gameobject with script control variables 有控制脚本的物体
private var outsideControls : controlHub;// making a link to corresponding bike's script

///////////////////////////////////////////////////  ESP system ////////////////////////////////////////////////////////
private var ESP : boolean = false;//ESP turned off by default		稳定器
...
//	ctrlHub = GameObject.Find("gameScenario");//link to GameObject with script "controlHub"
	outsideControls = GetComponent.<controlHub>();//to connect c# mobile control script to this one
	...
	GetComponent.<Rigidbody>().inertiaTensor = setInitialTensor;////this string is necessary for Unity 5.3 with new PhysX feature when Tensor decoupled from center of mass 惯性张量,描述转动惯性的量
	...
	//for better physics of fast moving bodies 插值允许你以固定的帧率平滑物理运行效果
	GetComponent.<Rigidbody>().interpolation = RigidbodyInterpolation.Interpolate;
	
	// too keep EngineTorque variable like "real" horse powers	让引擎扭矩变量更接近真正的马力
	...
	frontBrakePower = frontBrakePower * 30;//30 is abstract but necessary for Unity5 前轮刹车力
...
function FixedUpdate (){
	...
	// if RPM is more than engine can hold we should shift gear up		避免转速大于当前挡位的最大转速
	...
	ApplyLocalPositionToVisuals(coll_rearWheel);
 	...
 	//beauty - rear pendulumn is looking at rear wheel后悬挂跟着后轮旋转
 	rearPendulumn.transform.localRotation.eulerAngles.x = 0-8+(meshRearWheel.transform.localPosition.y*100);
 	//beauty - wheel bar rotating by front wheel	车把根据前轮旋转
	...
	if (!crashed){
		GetComponent.<Rigidbody>().drag = GetComponent.<Rigidbody>().velocity.magnitude / 210  * airRes; // when 250 bike can easy beat 200km/h // ~55 m/s		计算阻力
		GetComponent.<Rigidbody>().angularDrag = 7 + GetComponent.<Rigidbody>().velocity.magnitude/20;		//转动阻力
	}
	
	//determinate the bike speed in km/h
	bikeSpeed = Mathf.Round((GetComponent.<Rigidbody>().velocity.magnitude * 3.6)*10) * 0.1; //from m/s to km/h		四舍五入？
...
//////////////////////////////////// turnning /////////////////////////////////////////////////////////////			
		...
		//if (!crashed && Input.GetAxis("Horizontal") !=0){//DEL OLD
			...
			coll_frontWheel.steerAngle = tempMaxWheelAngle * outsideControls.Horizontal;	//偏移角度为最大偏移角*输入的horizontal（-1，1）
...
///////////////////////////////////////// CRASH happens /////////////////////////////////////////////////////////
	// conditions when crash is happen		//翻车，
...
function ApplyLocalPositionToVisuals (collider : WheelCollider) {
		...
		var visualWheel : Transform = collider.transform.GetChild (0);	//得到前（后）轮的子物体
		wheelCCenter = collider.transform.TransformPoint (collider.center);		//把自身坐标系下的该点的坐标转换为世界坐标
		if (Physics.Raycast (wheelCCenter, -collider.transform.up, hit, collider.suspensionDistance + collider.radius)) {
			visualWheel.transform.position = hit.point + (collider.transform.up * collider.radius); //轮子中心点位置为撞击物体碰撞器的上方加轮子半径
			if (collider.name == "coll_front_wheel") isFrontWheelInAir = false;		//判断前轮是否着地，仅在检测的是前轮的时候进行改变
			
		} else {	//射线没有检测到东西，如果检测的是前轮，那么前轮浮空
		...
		collider.GetWorldPose (position, rotation);		//得到该轮子在世界坐标下的position和rotation，参数即为输出（out）

		visualWheel.localEulerAngles = Vector3(visualWheel.localEulerAngles.x, collider.steerAngle - visualWheel.localEulerAngles.z, visualWheel.localEulerAngles.z);		//旋转角度
		visualWheel.Rotate (collider.rpm / 60 * 360 * Time.deltaTime, 0, 0);		//轮子绕轴旋转多少
...
//function SetHorizonAndVerticle(h : float,v : float){
//	outsideControls.Vertical = v;
//	outsideControls.Horizontal = h;
//}
